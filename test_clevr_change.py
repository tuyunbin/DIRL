...
with torch.no_grad():
    ...
    for i, batch in tqdm(enumerate(test_loader)):
        ...
        speaker_output_neg, neg_att = speaker.sample(diff_bef_neg, diff_aft_neg, sample_max=1)
        ...
        pos_dynamic_atts = pos_att.cpu().numpy()
        neg_dynamic_atts = neg_att.cpu().numpy()
        ...
        for j in range(batch_size):
            ...
            np.save(att_bef_path, pos_dynamic_atts[j])
            np.save(att_aft_path, pos_dynamic_atts[j])
